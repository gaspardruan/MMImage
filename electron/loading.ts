...
const getForeBackColor = () => {
  let foreColor = '#fff';
  let backColor = '#2f3241';

  const globalStorage = localStorage.getItem('global-storage');
  if (globalStorage) {
    const { theme } = JSON.parse(globalStorage).state;
...
export function loadingControl() {
  const { foreColor, backColor } = getForeBackColor();
...
.app-loading-wrap {
  ...
  oDiv.innerHTML = `<div class="sk-circle">
